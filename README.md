# Predictive-Analysis-of-Student-Success-Using-Machine-Learning
...
Education plays a vital role in shaping the future of societies. However, after the COVID-19 pandemic, global studies have shown a significant decline in students’ performance.  
To address this, machine learning can be used to analyze past academic records and predict the likelihood of student success or failure, enabling timely interventions by educators and institutions.

# Motivation

The education sector requires innovative, data-driven solutions to improve quality and reduce student failure rates.  
By analyzing previous academic data, we can identify patterns that affect student success, such as family background, health, or study habits.  
Machine learning provides a scalable way to extract such insights and build predictive models that guide educational strategies.

# Problematic

With the help of historical academic data, this project aims to:

1) Predict whether a student will pass the final exam.  
2) Identify which features most influence performance.  
3) Compare and select the best-performing model while avoiding overfitting and underfitting.  

This is a **classification problem**, where each student is classified as “Pass” or “Fail” based on features such as study time, parental education, absences, and social habits.
...
1) **Label Encoding:**  
   Convert categorical attributes into numerical form for machine learning algorithms.
...
2) **Feature Scaling:** 
Feature scaling is crucial for algorithms that rely on distance calculations or gradient-based optimization. Normalizing feature ranges improves **convergence speed** and **model performance**.
...
> **Note:** Binary columns (0/1) are not scaled.

---

##  Dataset Visualization

To understand the data, visualization techniques were applied using **matplotlib** and **seaborn**.

- **Distribution histograms** – Observe frequency counts (e.g., internet access, study time).  
- **Boxplots** – Detect outliers and view grade distributions across variables.  
- **Correlation heatmaps** – Identify the most influential variables on student status.

These visualizations help uncover patterns that are not easily detected through raw data analysis.

---

##  Model Evaluation (Metrics)

Before comparing models, the following **evaluation metrics** were defined:
...
### 3. ROC Curve
A plot showing the trade-off between **True Positive Rate (TPR)** and **False Positive Rate (FPR)**.
...
### 4. ROC-AUC Score
The **area under the ROC curve** — ideal value is **1.0**, representing perfect classification.
